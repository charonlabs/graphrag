...
async def __get_embedding_description_store(
    ...
    if config_args.get("overwrite", True):
        ...
        await store_entity_semantic_embeddings(
            entities=entities, vectorstore=description_embedding_store, session=session, graph_index=graph_index, vector_table_model=vector_table_model # type: ignore
        )
    else:
...
async def run_global_search(
    ...
    if use_db:
        ...
        assert graph_index is not None, "Graph Index is required when using the database."
    ...
    if not use_db:
        ...
        final_nodes = await emitter.load_table(name="create_final_nodes", graph_index=graph_index, session=session) # type: ignore
        final_entities = await emitter.load_table(name="create_final_entities", graph_index=graph_index, session=session) # type: ignore
        final_community_reports = await emitter.load_table(name="create_final_community_reports", graph_index=graph_index, session=session) # type: ignore
...
async def run_local_search(
    ...
    if not use_db:
        ...
        final_nodes = await emitter.load_table(name="create_final_nodes", graph_index=graph_index, session=session) # type: ignore
        final_community_reports = await emitter.load_table(name="create_final_community_reports", graph_index=graph_index, session=session) # type: ignore
        final_text_units = await emitter.load_table(name="create_final_text_units", graph_index=graph_index, session=session) # type: ignore
        final_relationships = await emitter.load_table(name="create_final_relationships", graph_index=graph_index, session=session) # type: ignore
        final_entities = await emitter.load_table(name="create_final_entities", graph_index=graph_index, session=session) # type: ignore
        try:
            final_covariates = await emitter.load_table(name="create_final_covariates", graph_index=graph_index, session=session) # type: ignore
        except:
    ...
    if isinstance(description_embedding_store, SupabaseVectorStore):
        result = await search_engine.asearch(query=query, session=session, graph_index=graph_index, vector_table_model=vector_table_model) # type: ignore
    else:
        result = await search_engine.asearch(query=query)
