...
class SupabaseVectorStore(BaseVectorStore):
    ...
    async def load_documents(
        ...
        if data:
            try:
                # Use execute instead of scalars for delete operation
                await session.execute(delete(vector_table_model).where(vector_table_model.index == graph_index)) # type: ignore
